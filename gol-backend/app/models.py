...
class User(Base):
    ...
    id = sa.Column(sa.VARCHAR(length=20), primary_key=True, nullable=False)
    create_at = sa.Column(sa.DATE, nullable=False)
    password = sa.Column(sa.VARCHAR(length=20), nullable=False)
    authority = sa.Column("authority", sa.Enum(
    ...
    email = sa.Column("email", sa.VARCHAR(length=100), nullable=False)
    member_balance = sa.Column("member_balance", sa.INTEGER)


class Game(Base):
    __tablename__ = "game"

    game_id = sa.Column(sa.VARCHAR(length=20),
    ...
    game_developer_id = sa.Column(sa.VARCHAR(
        length=20), sa.ForeignKey("user.Id"), nullable=False)


class Cart(Base):
    __tablename__ = "cart"

    cart_id = sa.Column(sa.VARCHAR(length=20),
                        primary_key=True, nullable=False)
    user_id = sa.Column(sa.VARCHAR(length=20),
                        sa.ForeignKey("user.Id"), nullable=False)
    game_id = sa.Column(sa.VARCHAR(length=20), sa.ForeignKey(
...
class GameList(Base):
    ...
    game_list_id = sa.Column(sa.VARCHAR(length=20),
                             primary_key=True, nullable=False)
    create_at = sa.Column(sa.DATE, nullable=False)
    user_id = sa.Column(sa.VARCHAR(length=20),
                        sa.ForeignKey("user.Id"), nullable=False)
...
class Issue(Base):
    ...
    issue_id = sa.Column(sa.VARCHAR(length=20),
                         primary_key=True, nullable=False)
    create_at = sa.Column(sa.DATE, nullable=False)
    ...
    refund_gameId = sa.Column(sa.VARCHAR(length=20),
